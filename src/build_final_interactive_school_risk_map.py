        ...
        records.append(
            {
                ...
                "supervisedPct": clean_float(row.get("final_supervised_closure_percentile"), 1),
...
def make_help_payload() -> str:
    help_text = {
        ...
        "diffScore": "높을수록 이 프로젝트의 차별 피처(고립도, 상권, 지역감소, EDSS 유사도)가 위험 쪽으로 강합니다.",
        "isolation": "높을수록 주변 같은 학교급이 멀거나 적어 교육공백 위험이 큽니다.",
        "commercial": "높을수록 주변 상권·교육·아동 관련 생활 기반이 약합니다.",
        "regionalDecline": "높을수록 출생, 출산율, 인구이동 흐름이 감소 방향입니다.",
...
<body>
  ...
  <div class="panel">
    ...
    <div class="subtitle">2026~2040 1년 단위 · 한국 좌표 범위 밖 이상값 제외 · 학교고립도/상권취약도/지역감소위험 표시</div>
    ...
    <div class="feature-controls" id="featureControls">
      ...
      <label><input type="checkbox" class="feature-toggle" data-feature="regional" checked> 지역 감소위험</label>
      <label><input type="checkbox" class="feature-toggle" data-feature="edss" checked> EDSS 유사위험</label>
      <label><input type="checkbox" class="feature-toggle" data-feature="replacement" checked> 대체학교 접근성</label>
      <div class="feature-note">자동 전환은 넓게 볼 때 시군구 감소 버블, 확대 시 학교별 점으로 바뀝니다. 학교별 점의 숫자 동그라미는 가까운 학교를 묶은 클러스터 수입니다.</div>
    </div>
    <div class="model-summary">
      ...
      <div class="model-proxy-note"><span class="tag proxy">참고</span>분류는 EDSS 학교ID 소멸 proxy 기반 보조모델이며, 최종 위험점수 100점 중 15점만 기여합니다.</div>
    </div>
  ...
  <div class="graph-panel" id="graphPanel">
    ...
    <div class="graph-model-cards">
      ...
      <div class="graph-model-card">
        <div class="card-title">분류모델: EDSS 소멸패턴</div>
        <div class="card-compare">
          <div class="card-item base">
            <div class="card-tag">베이스</div>
            <div class="card-name">Logistic</div>
            <div class="card-metric">F1 0.439</div>
          </div>
          <div class="card-vs">vs</div>
          <div class="card-item tuned">
            <div class="card-tag">튜닝</div>
            <div class="card-name">HistGB</div>
            <div class="card-metric">F1 0.545</div>
            <div class="card-badge">24.1% 개선</div>
          </div>
        </div>
        <div class="card-note">EDSS proxy 기반 보조지표이며 최종 위험점수 일부로만 사용됩니다.</div>
      </div>
    </div>
    <div class="graph-controls">
      ...
      <div class="graph-feature-controls" id="graphFeatureControls">
        ...
        <label><input type="checkbox" class="feature-toggle graph-feature-toggle" data-feature="regional" checked> 지역 감소위험</label>
        <label><input type="checkbox" class="feature-toggle graph-feature-toggle" data-feature="edss" checked> EDSS 유사위험</label>
  ...
  <script>
    ...
    function riskContributions(d) {{
      ...
      const items = [
        ...
        {{ label: '지역 감소위험 높음', active: features.regional && d.flagRegionalDeclineHigh === 1, points: RISK_WEIGHTS.regional }},
        {{ label: 'EDSS 유사도 상위 10%', active: features.edss && d.flagObjectiveTop10 === 1, points: RISK_WEIGHTS.edss }},
    ...
    function popup(d) {{
      ...
      const delta = (d.students2025 && view.forecast !== null && view.forecast !== undefined)
        ...
        : '-';
      const baseTone = metricTone(view.score, 60, 35);
      ...
      return `<div class="popup">
        ...
        <div class="metric-grid">
          ${{htmlMetric('전체 피처 기준', view.score, '점', `${{view.riskKo}} · ${{view.basisLabel}}`, baseTone)}}
          ...
          ${{htmlMetric('지역 감소위험', d.regionalDecline, '', '높을수록 인구 감소 흐름 강함', regionalTone)}}
    ...
    async function render() {{
      ...
      const modelText = selectedForecastModel() === 'pressure' ? '기존 시군구 압력비 기준' : '출생 코호트 기준';
