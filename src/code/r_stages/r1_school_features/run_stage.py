...
def run_r1(input_path: Path, output_dir: Path, horizon: int) -> pd.DataFrame:
    """Train and evaluate R1 school features modeling stage."""
    ...
    features = infer_feature_columns(df, "R1")
    ...
    write_csv(metrics_df, output_dir / f"r1_school_features_{horizon}yr_metrics.csv")
    
    if pred_rows:
        write_csv(pd.concat(pred_rows, ignore_index=True), output_dir / f"r1_school_features_{horizon}yr_predictions.csv")
        
    return metrics_df

def main() -> None:
    parser = argparse.ArgumentParser(description="Run R1 Modeling")
    parser.add_argument("--input", type=str, default=str(CLEAN_PATCH_DIR / "model_views" / "r1_basic_1yr.csv"))
    parser.add_argument("--output-dir", type=str, default=str(POLICY_COMP_DIR / "results"))
    parser.add_argument("--horizon", type=int, default=1)
    args = parser.parse_args()
    
    run_r1(Path(args.input), Path(args.output_dir), args.horizon)
    print(f"R1 modeling completed for horizon={args.horizon}yr.")
