...
def run_r2(input_path: Path, output_dir: Path, horizon: int) -> pd.DataFrame:
    """Train and evaluate R2 spatial isolation modeling stage."""
    ...
    features = infer_feature_columns(df, "R2")
    ...
    write_csv(metrics_df, output_dir / f"r2_isolation_{horizon}yr_metrics.csv")
    
    if pred_rows:
        write_csv(pd.concat(pred_rows, ignore_index=True), output_dir / f"r2_isolation_{horizon}yr_predictions.csv")
        
    return metrics_df

def main() -> None:
    parser = argparse.ArgumentParser(description="Run R2 Modeling")
    parser.add_argument("--input", type=str, default=str(CLEAN_PATCH_DIR / "model_views" / "r2_isolation_1yr.csv"))
    parser.add_argument("--output-dir", type=str, default=str(POLICY_COMP_DIR / "results"))
    parser.add_argument("--horizon", type=int, default=1)
    args = parser.parse_args()
    
    run_r2(Path(args.input), Path(args.output_dir), args.horizon)
    print(f"R2 modeling completed for horizon={args.horizon}yr.")
