...
def permutation_feature_importance(
    ...
    test = data[data["base_year"].between(test_year_min, test_year_max)].dropna(subset=[target_col]).copy()
...
def main() -> int:
    ...
    classifier = joblib.load(MODELS / "edss_closure_proxy_classifier_national.pkl")
    clf_imp = permutation_feature_importance(
        classifier,
        ROOT / "data" / "processed" / "edss_national_closure_proxy_dataset.csv",
        "closed_within_5yr_proxy",
        2016,
        2018,
        "classification_feature_importance.csv",
    )
    save_bar(clf_imp, "Classification Feature Importance", FIGURES / "classification_feature_importance.png")

    context_classifier = joblib.load(MODELS / "final_supervised_closure_classifier.pkl")
    context_imp = permutation_feature_importance(
        context_classifier,
        ROOT / "data" / "processed" / "final_supervised_closure_training_dataset.csv",
        "closed_within_5yr_proxy",
        2016,
        2018,
        "classification_context_feature_importance.csv",
    )
    save_bar(
        context_imp,
        "Classification Context Feature Importance",
        FIGURES / "classification_context_feature_importance.png",
    )
    ...
    print(clf_imp.head(15).to_string(index=False))
    print("\nclassification context top features")
