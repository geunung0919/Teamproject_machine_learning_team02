...
def train_objective_classifier_with_context() -> pd.DataFrame:
    # Keeps the objective EDSS classifier as an audited benchmark.
    # Context-rich policy labels are produced from the scenario, because historical EDSS lacks school coordinates/sgg.
    existing = pd.read_csv(REPORTS / "edss_closure_classifier_metrics_national.csv")
    existing["model_note"] = "objective EDSS 5-year school-id disappearance classifier; context features applied in policy scenario"
    existing.to_csv(REPORTS / "final_national_objective_classifier_metrics.csv", index=False, encoding="utf-8-sig")
    return existing
