...
def train_classifier(dataset: pd.DataFrame) -> tuple[Pipeline, pd.DataFrame, str]:
    ...
    pd.DataFrame(per_sido).sort_values("pr_auc", ascending=False).to_csv(
        REPORTS / "auxiliary_edss_similarity_metrics_by_sido.csv", index=False, encoding="utf-8-sig"
    )
    test_out.to_csv(REPORTS / "auxiliary_edss_similarity_test_predictions.csv", index=False, encoding="utf-8-sig")
...
def main() -> int:
    ...
    metrics.to_csv(REPORTS / "auxiliary_edss_similarity_model_metrics.csv", index=False, encoding="utf-8-sig")
    (REPORTS / "auxiliary_edss_similarity_model_report.txt").write_text(report, encoding="utf-8")
