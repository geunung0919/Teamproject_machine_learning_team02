...
def save_classifier_comparison() -> None:
    metrics = pd.read_csv(REPORTS / "temporal_closure_classifier_metrics.csv")
    label_map = {
        "base_logistic_temporal_closure": "Base\nLogistic",
        "tuned_histgb_temporal_closure": "Tuned\nHistGB",
    }
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title("분류모델 비교: 2019~2022 EDSS 소멸 검증", fontweight="bold")
